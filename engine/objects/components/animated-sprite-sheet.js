...
export default class AnimatedSprite extends Component{
  ...
  get src(){
    return this.#animations[this.#current].src;
  }
  ...
  update(){
    ...
    if(Time.frameCount % frameRate === 0){
      ...
      this.frame = (this.frame + 1) % collumns;
      this.slice.position.set(X, Y);
      this.slice.size.set(cellSize);
  ...
  validateAnimation(animation){
    ...
    if(!animation.src) throw new Error("Animation src is required");
    if(!animation.frameRate) throw new Error("Animation frameRate is required");
    if(!animation.collumns) throw new Error("Animation collumns is required");
    if(isNaN(animation.rowIndex)) throw new Error("Animation rowIndex is required");
